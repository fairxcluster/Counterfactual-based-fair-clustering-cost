...
def mbdmb_lp_solver(distance, num_centers, frac_list, frac_lp_assgn, color_flag):
    ...
    objective, lower_bounds, upper_bounds, variable_names = prepare_to_add_variables(
        distance, num_centers, frac_list, frac_lp_assgn
    )

    variables = {}
    for name, obj, lb, ub in zip(variable_names, objective, lower_bounds, upper_bounds):
        variables[name] = model.addVar(lb=lb, ub=ub, obj=obj, name=name)
    t2 = time.monotonic()
    print("Completed. Time for creating and adding variables = {}".format(t2 - t1))

    # Step 2: Add constraints
    print("Starting to add constraints...")
    t1 = time.monotonic()
    constraints_row, senses, rhs, constraint_names = prepare_to_add_constraints(
        num_centers, frac_list, frac_lp_assgn, color_flag
    )

    for row, sense, rhs_val, name in zip(constraints_row, senses, rhs, constraint_names):
        var_names, coefs = row
        expr = gp.LinExpr()
        for var, coef in zip(var_names, coefs):
            expr.add(variables[var], coef)

        if sense == "E":
            model.addConstr(expr == rhs_val, name=name)
        elif sense == "L":
            model.addConstr(expr <= rhs_val, name=name)
        elif sense == "G":
            model.addConstr(expr >= rhs_val, name=name)
    ...
    assignment_values = [variables[name].X for name in variable_names]

    res = {
        "status": model.Status,
        "success": "Optimal" if model.Status == GRB.OPTIMAL else "Not Optimal",
