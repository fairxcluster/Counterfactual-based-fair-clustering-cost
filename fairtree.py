...
def kmedian_cost(points, centroids, dataset):
    ...
    return sum(np.amin(np.concatenate([np.linalg.norm(dataset[:,:]-dataset[centroid,:], axis=1,ord=order).reshape((dataset.shape[0], 1)) for centroid in centroids], axis=1), axis=1))
...
def fair_kmedian_cost(centroids, dataset):
    ...
    for i in range(len(FAIRLETS)):
        ...
        cost_list = [np.linalg.norm(dataset[centroids[j],:]-dataset[FAIRLET_CENTERS[i],:],ord=order) for j in range(len(centroids))]
        cost, j = min((cost, j) for (j, cost) in enumerate(cost_list))
        # Assign all points in i-th fairlet to above centroid and compute cost
        total_cost += sum([np.linalg.norm(dataset[centroids[j],:]-dataset[point,:],ord=order) for point in FAIRLETS[i]])
    return total_cost
...
def make_fairlet(points, dataset):
    ...
    cost_list = [sum([np.linalg.norm(dataset[center,:]-dataset[point,:],ord=order) for point in points]) for center in points]
...
def tree_fairlet_decomposition(p, q, root, dataset, colors):
    ...
    return node_fairlet_decomposition(p, q, root, dataset, donelist, 0), FAIRLET_CENTERS , FAIRLETS
